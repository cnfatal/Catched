...
| Abnormal ArtMethod struct size                 | Address delta between adjacent methods            | Size is non-positive, far outside a conservative safety range (for example > 2KB), or sharply deviates from a locally calibrated baseline                            |
...
1. **Measure ArtMethod size** — Select a class known to have multiple methods (e.g., `java.lang.String`). Use JNI `FromReflectedMethod()` to obtain pointers for two methods (e.g., `length()` and `isEmpty()`). Calculate the address difference as a heuristic signal only. On modern Android devices this value can vary significantly across OS versions, OEM ROMs, and ART layouts; use a conservative safety range and prefer per-device baseline calibration over fixed constants.
