...
5. **Generate DEX via DexBuilder at runtime** — NPatch and similar frameworks use `com.android.dex.DexBuilder` or equivalent bytecode generation APIs to create DEX bytecode in memory that wraps native method calls. The generated DEX is loaded via `InMemoryDexClassLoader`, leaving no file on disk but still requiring a ClassLoader to host the generated classes.
6. **Redirect RegisterNatives** — Instead of modifying ArtMethod fields, the attacker hooks the JNI `RegisterNatives` call to intercept native method registration. The hook substitutes custom native function pointers that redirect to the attacker's handler. This creates new method bindings visible through ClassLoader enumeration of loaded classes.
...
| DexBuilder-generated class               | ClassLoader loaded classes                          | Classes with synthetic names (numeric suffixes, single-letter packages) generated by runtime DEX construction |
| RegisterNatives-injected binding         | JNI method registration table                       | Native methods registered to function pointers outside the declaring library's address range                  |
| InMemoryDexClassLoader with no file path | `dexFile.getName()` returns synthetic name          | DEX loaded from memory buffer; no backing file exists on disk — common for runtime-generated hook stubs       |
