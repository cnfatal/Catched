...
int check_inline_hook(void *func_addr)
{
    ...
    // 模式 1: Dobby — LDR X17, #8; BR X17 (51 00 00 58 20 02 1F D6)
    if (insn0 == 0x58000051 && insn1 == 0xD61F0220)
    {
        LOGD("Inline hook (Dobby): LDR X17+BR X17 at %p", func_addr);
        return 1;
    }

    // 模式 2: android-inline-hook — LDR X16, #8; BR X16 (50 00 00 58 00 02 1F D6)
    if (insn0 == 0x58000050 && insn1 == 0xD61F0200)
    {
        LOGD("Inline hook (shadowhook/inline-hook): LDR X16+BR X16 at %p", func_addr);
        return 1;
    }

    // 模式 3: ShadowHook — STP X16,X17,[SP,#-0x10]!; LDR X17,#8; BR X17
    if (insn0 == 0xA9BF47F0 /* STP X16,X17,[SP,#-0x10]! */ &&
        insn1 == 0x58000051 /* LDR X17, #8 */)
    {
        uint32_t insn2 = code[2];
        if (insn2 == 0xD61F0220)
        { // BR X17
            LOGD("Inline hook (ShadowHook): STP+LDR+BR at %p", func_addr);
            return 1;
        }
    }

    // 模式 4: 通用 LDR Xn, =addr (LDR literal) → BR Xn
    if ((insn0 & 0xFF000000) == 0x58000000)
    { // LDR Xn, #imm
        if ((insn1 & 0xFFFFFC1F) == 0xD61F0000)
        { // BR Xn
            LOGD("Inline hook (generic): LDR+BR at %p", func_addr);
            return 1;
        }
    }
    ...
    {
        LOGD("Inline hook: B instruction at %p", func_addr);
        return 1;
    }
    ...
    { // ADRP Xn, #page
        ...
        { // ADD Xn, Xn, #off
            uint32_t insn2 = code[2];
            if ((insn2 & 0xFFFFFC1F) == 0xD61F0000)
            { // BR Xn
                LOGD("Inline hook: ADRP+ADD+BR at %p", func_addr);
                return 1;
            }
        }
    }
    ...
    uint32_t *code = (uint32_t *)((uintptr_t)func_addr & ~1);
    uint32_t insn = code[0];

    // 模式 1: LDR PC, [PC, #-4] — 04 F0 1F E5 (Dobby, ShadowHook ARM32)
    if (insn == 0xE51FF004)
    {
        LOGD("Inline hook: LDR PC,[PC,#-4] at %p", func_addr);
        return 1;
    }

    // 模式 2: LDR PC, [PC, #0] — 00 F0 9F E5 (android-inline-hook ARM32)
    if (insn == 0xE59FF000)
    {
        LOGD("Inline hook: LDR PC,[PC,#0] at %p", func_addr);
        return 1;
    }

    // 模式 3: 通用 LDR PC, [PC, #offset]
    ...
    // 模式 4: Thumb — LDR.W PC, [PC, #0] — DF F8 00 F0
    if ((uintptr_t)func_addr & 1)
    { // Thumb 模式
        uint16_t *thumb = (uint16_t *)((uintptr_t)func_addr & ~1);
        if (thumb[0] == 0xF8DF && thumb[1] == 0xF000)
        {
            LOGD("Inline hook: Thumb LDR.W PC at %p", func_addr);
            return 1;
        }
    }
#endif
...
int check_critical_functions_hook(void)
{
    ...
    {
        void *sym = dlsym(libc, targets[i]);
        if (sym && check_inline_hook(sym))
        {
            LOGD("Critical function hooked: %s at %p", targets[i], sym);
            hooked++;
        }
    }
